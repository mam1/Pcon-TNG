/********************************************************************/
/*	Dcon.c - daemon updates the channel once a minute unless 		*/
...
int            	ipc;                        	// file descriptor for ipc data file
...
// 		switch (ipc_ptr->ipc_dat.c_dat[channel].mode) {
// 		case 0:	// manual
// 			state = ipc_ptr->ipc_dat.c_dat[channel].state;
// 			if(state)
// 			{	current_time = clock();
...
// 			break;
// 		case 2:	// time & sensor
// 			state =  test_sch_sensor(key, &(ipc_ptr->sys_data.sys_sch.sch[tm->tm_wday][channel]), ipc_ptr->s_dat[ipc_ptr->ipc_dat.c_dat[channel].sensor_id].temp);
// 			if(state)
// 			{	current_time = clock();
// 				for ( ; (clock() - current_time) < (2 * CLOCKS_PER_SEC); );
// 			}
...
// 		}
// 		ipc_ptr->ipc_dat.c_dat[channel].state = state;
// 	}

// 	/* update gpio pins for all channels */
// 	logit("starting pin update");
// 	for (channel = 0; channel < _NUMBER_OF_CHANNELS; channel++) {
// 		if (ipc_ptr->ipc_dat.c_dat[channel].state) {
...
// This function loads the time date from the system time
void get_tm(struct tm *pt)
{
	time_t 			t;
	struct tm 		st;
...
printf("%s\n", "ready to launch daemon" );
	...
	ipc_sem_lock(semid, &sb);					// wait for a lock on shared memory
	ipc = ipc_open(ipc_file, ipc_size());      	// create/open ipc file
	data = ipc_map(ipc, ipc_size());           	// map file to memory
